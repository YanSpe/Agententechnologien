...
class CollectAgent(collectID: String, obstacles: List<Position>?, repairPoints: List<Position>, size: Position) :
        ...
        if (!hasMaterial && !vision.isEmpty()) {
            ...
            ref invoke ask<WorkerActionResponse>(WorkerActionRequest(this.collectID, nextAction)) {
                if (it.state) {
                    ...
                    considerActionFlags(it, position)
                }
            }
    ...
    private fun doMove(position: Position) {
        val ref = system.resolve(SERVER_NAME)
    ...
    private fun considerActionFlags(message: WorkerActionResponse, position: Position) {
        when (message.flag) {
            ...
            ActionFlag.OBSTACLE -> doMove(position)
            ActionFlag.NONE -> log.info(collectID + ": None")
            ActionFlag.NO_MATERIAL -> doMove(position)
        }
    }
